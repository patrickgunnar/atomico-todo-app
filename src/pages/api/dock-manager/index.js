import folder_db_handler_dock from '../mongodb/folder_db_handler_dock'
import list_db_handler_dock from '../mongodb/list_db_handler_dock'
        ...
        if (where === 'folder-handler') {
            ...
            if (action && ids && loopOverData && dataToInsert) {
                try {
                    ...
                    if(user.user !== 'no-current-user' && user.email !== 'no-current-email') {
                        returnData = await folder_db_handler_dock(action, ids, dataToInsert, user)
                        // no user profile
                        // operates locally
                    } else {
                        returnData = folder_handler(action, ids, dataToInsert, JSON.parse(loopOverData))
                    }
            ...
            if (action && ids && loopOverData && dataToInsert) {
                try {
                    ...
                    // operates on db
                    if(user.user !== 'no-current-user' && user.email !== 'no-current-email') {
                        returnData = await list_db_handler_dock(action, ids, dataToInsert, user)
                        // no user profile
                        // operates locally
                    } else {
                        returnData = list_handler(action, ids, dataToInsert, JSON.parse(loopOverData))
                    }
