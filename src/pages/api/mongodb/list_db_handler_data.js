    ...
    try {
        ...
        if (action === 'add-task') {
            ...
            if (action === 'delete-task') {
                ...
                const result = await collection.updateOne({ id: listId, 'content.id': taskId }, {
                    $set: {
                        'content.$[i].text': dataToInsert.text,
                        'content.$[i].date': dataToInsert.date
                    }
                }, {
                    arrayFilters: [{
                        'i.id': taskId
                ...
                // if the user request is to favourite task
            } else if (action === 'favourite-task') {
                const result = await collection.updateOne({ id: listId, 'content.id': taskId }, {
                    $set: {
                        'content.$[i].favourite': !dataToInsert.isFav
                    }
                }, {
                    arrayFilters: [{
                        'i.id': taskId
